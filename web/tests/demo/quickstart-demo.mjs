...
const demoEmail = process.env.DEMO_EMAIL || 'demo@beebuzz.local';
...
const dashboardVideoDir = path.join(outputDir, 'dashboard-video');
const hiveVideoDir = path.join(outputDir, 'hive-video');
const dashboardVideoOutput = path.join(dashboardVideoDir, 'quickstart-demo-dashboard.webm');
...
const dashboardContext = await launchWindow('dashboard', 40, path.join(userDataRoot, 'dashboard'));
...
await dashboardContext.grantPermissions(['clipboard-read', 'clipboard-write'], { origin: dashboardURL });
await hiveContext.grantPermissions(['clipboard-read', 'clipboard-write', 'notifications'], {
  origin: hiveURL
});

const dashboardPage = dashboardContext.pages()[0] ?? (await dashboardContext.newPage());
...
try {
  logStep('opening Dashboard and Hive');
  await Promise.all([dashboardPage.goto(`${dashboardURL}/auth`), hivePage.goto(`${hiveURL}/pair`)]);
  await pause(1.5);

  logStep('switching Hive to browser fallback');
  await useHiveBrowserFallback(hivePage);

  logStep('signing in on Dashboard');
  await humanFill(dashboardPage.getByLabel('Email address'), demoEmail);
  await humanClick(dashboardPage.getByRole('button', { name: /continue/i }));

  await expect(dashboardPage).toHaveURL(/\/auth\/verify/);
  const otp = await latestOTP();
  await pause();
  logStep('entering OTP');
  await humanFill(dashboardPage.getByLabel('One-time code'), otp);
  await humanClick(dashboardPage.getByRole('button', { name: /verify code/i }));
  await expect(dashboardPage).toHaveURL(/\/account\/overview/, { timeout: 15000 });

  logStep('creating pairing code');
  await pause(1.2);
  await dashboardPage.goto(`${dashboardURL}/account/devices`);
  await pause(1.2);
  await humanClick(dashboardPage.getByRole('button', { name: /add device/i }));
  await humanFill(dashboardPage.getByLabel('Device Name'), 'Demo Chrome Hive');
  await humanClick(dashboardPage.getByRole('button', { name: /generate pairing code/i }));

  const pairingDialog = dashboardPage.getByRole('dialog', { name: /pairing code generated/i });
  ...
  await expect(dashboardPage.getByText(/pairing done/i)).toBeVisible({ timeout: 15000 });
  await humanClick(dashboardPage.getByRole('button', { name: /go to api tokens/i }));

  logStep('creating API token');
  await expect(dashboardPage).toHaveURL(/\/account\/api-tokens/);
  await pause(1.2);
  await humanClick(dashboardPage.getByRole('button', { name: /create token/i }).first());
  await humanFill(dashboardPage.getByLabel('Token Name'), 'Quickstart demo token');
  ...
  const createDialog = dashboardPage.getByRole('dialog', { name: /create api token/i });
  ...
  const tokenDialog = dashboardPage.getByRole('dialog', { name: /api token created/i });
  ...
  await humanClick(tokenDialog.getByRole('button', { name: /send test message now/i }));
  ...
  await pause(2);
  await dashboardPage.screenshot({ path: path.join(outputDir, 'quickstart-demo-dashboard-final.png') });
  ...
  const dashboardVideo = dashboardPage.video();
  const hiveVideo = hivePage.video();
  await dashboardContext
    .close()
    .catch((e) => console.error(`[quickstart-demo] dashboard close error: ${formatError(e)}`));
  await hiveContext
    .close()
    .catch((e) => console.error(`[quickstart-demo] hive close error: ${formatError(e)}`));
  await persistVideo(dashboardVideo, dashboardVideoOutput).catch((e) =>
    console.error(`[quickstart-demo] dashboard video move error: ${formatError(e)}`)
  );
